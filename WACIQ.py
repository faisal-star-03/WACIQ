...
def animated_logo(logo, width=50, speed=0.02):
    """لوګو animation: slide + typewriter + رنگ"""
    logo_lines = logo.split('\n')
    shift = width
    direction = -1  # حرکت د ښي څخه چپ
    while True:
        os.system('clear')
        ...
        if shift < 0 or shift > width:
            direction *= -1  # د حرکت direction بدلول

# دا thread د animation لپاره
def start_logo_thread():
    t = threading.Thread(target=animated_logo, args=(logo,))
    t.daemon = True
    t.start()

# بیا خپل موجوده Jaber() function څخه مخکې یي call کړه
start_logo_thread()                                                                                                               
